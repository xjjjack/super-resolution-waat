...
#from model.uper_resolution import Faster_SR_Net, Faster_SR_Net_F
#from model.SR_models import Bicubic_plus_plus
...
def preprocess_image_cv(image_path):
    ...
    x_slices = math.ceil(width / 48)
    y_slices = math.ceil(height / 48)

    for i in range(y_slices):
        for j in range(x_slices):
            left = j * 48
            top = i * 48
            right = min((j + 1) * 48, width)
            bottom = min((i + 1) * 48, height)
...
def postprocess_images_cv(slices, original_size, upscale_factor):
    ...
    x_slices = math.ceil(upscaled_width / (48 * upscale_factor))
    y_slices = math.ceil(upscaled_height / (48 * upscale_factor))
    
    current_slice = 0
    for i in range(y_slices):
        for j in range(x_slices):
            if current_slice < len(slices):
                top = i * 48 * upscale_factor
                left = j * 48 * upscale_factor
...
model = ESRT(upscale=3).to(device)
#model = Faster_SR_Net(upscale_factor=3).to(device)
#model = Faster_SR_Net_F(upscale_factor=3).to(device)
#state_dic = torch.load(r"G:\ECE1508\project\my_own_solution\checkpoint\2024_03_21\model\2024-03-21-08-52-19.pth")
#model = Bicubic_plus_plus(sr_rate=3).to(device)
#state_dic = torch.load(r"G:\ECE1508\project\my_own_solution\checkpoint\2024_03_21_1\model\2024-03-21-16-26-33.pth")
state_dic = torch.load(r"G:\ECE1508\project\ESRT\checkpoint\3x\epoch_1000.pth")
