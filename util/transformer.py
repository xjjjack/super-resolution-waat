...
from util.tools import extract_image_patches_same,\
...
class MLABlock(nn.Module):
    ...
    def forward(self, x):
        ...
        B = x.shape[0]
        x = extract_image_patches_same(x, ksizes=torch.tensor([3, 3]), strides=torch.tensor([1,1]), rates=torch.tensor([1, 1]))
        x = self.unfold(x)
        x = x.permute(0,2,1)
